...
//This code will allow you to control up to 16 Atlas Scientific devices through 1 soft serial RX/TX line.
//This code was written in the Arduino 1.8.9 IDE
//An Arduino UNO was used to test this code.
//This code was last tested 5/2019


//To open a channel (marked on the board as P1 to P8, the second port expander ports are 9-16) send the number of the channel followed by a colon and the command (if any) that you want to send. End the string with a carriage return.
//1:r<CR>
//2:i<CR>
//3:c<CR>
//4:r<CR>
...
int EN2 = 8;                                          //Arduino pin 8 to control pin EN pin on the second serial port expander
int EN1 = 7;                                          //Arduino pin 7 to control pin EN pin on the first serial port expander
...
void setup() {
  ...
  pinMode(EN1, OUTPUT);                               //Set the digital pin as output
  pinMode(EN2, OUTPUT);                               //Set the digital pin as output
  digitalWrite(EN1, LOW);                             //Set the EN pins to a know state
  digitalWrite(EN2, HIGH);                            //Set the EN pins to a know state
}
...
void open_port() {                                   //this function controls what UART port is opened.
  ...
  if (port < 1 || port > 16)port = 1;                //if the value of the port is within range (1-16) then open that port. If it’s not in range set it to port 1
  ...
  if (port <= 7) {                                   //if port <=7 then we are trying to open a port on the first serial port expander
    digitalWrite(EN1, LOW);                          //enable the first serial port expander
    digitalWrite(EN2, HIGH);                         //disable the second serial port expander
  }
  else {                                            //if port >7 then we are trying to open a port on the second serial port expander
    port -= 8;                                      //lets get the port number back in range (example port =15 subtract 8, port = 7)
    digitalWrite(EN1, HIGH);                        //disable the first serial port expander
    digitalWrite(EN2, LOW);                         //enable the second serial port expander
  }
